...
def generate_models_dict(models, sim_names, user_based):
    ...
    for sim_name in sim_names:
        sim_dict = {"name": sim_name, "user_based": user_based}
        for model in models:
...
print(
    "Number of ratings: %d\nNumber of books: %d\nNumber of users: %d"
    % (len(df), len(df["isbn"].unique()), len(df["user_id"].unique()))
)

models1 = generate_models_dict(
    [KNNBasic, KNNWithMeans, KNNWithZScore, KNNBaseline], ["pearson"], True
)
