...
def pre_process():
    books = pd.read_csv(
        "data/BX_Books.csv", sep=";", error_bad_lines=False, encoding="latin-1"
    )
    books.columns = [
        "ISBN",
        "Book-Title",
    ...
    users = pd.read_csv(
        "data/BX-Users.csv", sep=";", error_bad_lines=False, encoding="latin-1"
    )
    users.columns = ["User-ID", "Location", "Age"]
    ratings = pd.read_csv(
        "data/BX-Book-Ratings.csv", sep=";", error_bad_lines=False, encoding="latin-1"
    )
    ...
    filter_users_list = filter_users[
        filter_users >= user_ratings_threshold
    ].index.to_list()

    df_ratings_top = ratings_explicit[
        ratings_explicit["user_id"].isin(filter_users_list)
    ]

    print(
        "Filter: users with at least %d ratings\nNumber of records: %d"
        % (user_ratings_threshold, len(df_ratings_top))
    )

    book_ratings_threshold_perc = 0.1
    book_ratings_threshold = (
        len(df_ratings_top["isbn"].unique()) * book_ratings_threshold_perc
    )

    filter_books_list = (
        df_ratings_top["isbn"]
        .value_counts()
        .head(int(book_ratings_threshold))
        .index.to_list()
    )
    ...
    print(
        ...
        % (book_ratings_threshold_perc * 100, len(df_ratings_top))
    )
